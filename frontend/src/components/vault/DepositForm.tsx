...
export function DepositForm() {
  ...
  async function handleDeposit() {
    ...
    setLoading(true);
    const toastId = txPending("Depositing USDC...");

    try {
      const raw = parseUsdc(amount);
      const result = await VaultContract.deposit(address, raw);
      toast.dismiss(toastId);
      txSuccess(result.hash, "USDC deposited successfully");
      setAmount("");
      setSelectedPreset(null);
    } catch (err) {
      toast.dismiss(toastId);
      txError(err instanceof Error ? err.message : "Deposit failed");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <Button
        ...
        disabled={loading}
        className="btn-depth group w-full"
        size="lg"
      >
        {!connected ? (
          <>
